...
5. **Target Return Rate Comparison & Account Management:** Compares actual performance against compounded target annual rates per account with ahead/behind metrics. Features an inline "Edit Account" dialog to customize target rates, names, and metadata, as well as a red "Delete Account" button with irreversible confirmation, full cascading database cleanup (snapshots, holdings, transactions, and linked loans), success confirmation, error handling, and instant sidebar refresh.
